...
/**
 * The function `invokeSorobanOperation` invokes a Stellar Soroban operation, parses the result, and
 * returns it as a promise.
 * @param {string} operation - The `operation` parameter is the operation to invoke in base64 XDR
 * format. It represents the specific operation that you want to perform using the Stellar Soroban
 * protocol. This operation will be decoded and executed within the context of the Stellar network.
 * @param parser - The `parser` parameter is a function that takes a string as input and returns a
 * parsed result of type `T`. It is used to parse the result of the Stellar Soroban operation after it
 * has been invoked.
 * @param {TxParams} txParams - TxParams is an object containing transaction parameters such as account
 * information, transaction builder options, and other necessary data for executing a Stellar
 * transaction. It likely includes properties like `accountId`, `txBuilderOptions`, and other relevant
 * fields needed for constructing and executing a Stellar transaction.
 * @param [sorobanData] - The `sorobanData` parameter in the `invokeSorobanOperation` function is an
 * optional parameter of type `xdr.SorobanTransactionData`. It allows you to provide additional Soroban
 * transaction data if needed when invoking the Stellar Soroban operation. If you
 */
